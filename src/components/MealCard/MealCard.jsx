...
const MealCard = ({ id, image, title, isFav, onClickFavorites }) => {
    ...
    return (
        <Link to={PATHS.getDetailsUrl(id)} className="shadow-md rounded-lg relative hover:shadow-cocoa transition-shadow duration-300 ease-in-out">
            <img 
                src={image} 
                alt={title} 
                className="w-full object-cover rounded-lg aspect-square inline-block  hover:shadow-2xl transition-shadow duration-300 ease-in-out"
            />
